...
function muatDataFasilitas() {
    try {
        ...
        if (Array.isArray(dataLokal)) {
            ...
            logConsole(`${daftarFasilitas.length} fasilitas dimuat dari localStorage`, 'success');
        } else {
            daftarFasilitas = [];
...
function tambahFasilitas(dataFasilitas) {
    try {
        ...
        // Notifikasi sukses
        tampilkanNotifikasi('success', `Fasilitas "${fasilitasBaru.nama}" berhasil ditambahkan!`);
        ...
        tampilkanNotifikasi('error', 'Gagal menambah fasilitas. Silakan coba lagi.');
        return false;
    }
}
...
function updateFasilitas(id, dataBaru) {
    try {
        ...
        // Notifikasi sukses
        tampilkanNotifikasi('success', `Fasilitas "${daftarFasilitas[index].nama}" berhasil diupdate!`);
...
async function hapusFasilitas(id) {
    try {
        ...
        // Notifikasi sukses
        tampilkanNotifikasi('success', `Fasilitas "${fasilitas.nama}" berhasil dihapus!`);
...
function renderTabelFasilitas() {
    ...
    dataHalamanIni.forEach((fasilitas, idx) => {
        ...
        htmlRows += `
      <tr data-id="${fasilitas.id}" onclick="highlightRowTabel('${fasilitas.id}')">
        <td>${nomer}</td>
        <td><strong>${escapeHTML(fasilitas.nama)}</strong></td>
        <td>
          <span class="badge" style="background: ${getWarnaKategori(fasilitas.kategori)}">
            ${escapeHTML(fasilitas.kategori)}
          </span>
        </td>
        <td>${escapeHTML(fasilitas.alamat || '-')}</td>
        <td>
          <small>${formatKoordinat(fasilitas.koordinat.lat, fasilitas.koordinat.lng, 4)}</small>
        </td>
        <td>
          <div class="action-buttons">
            <button class="btn-action btn-view" onclick="event.stopPropagation(); panKeMarker('${fasilitas.id}')" title="Lihat di Peta">
              <i data-lucide="eye"></i>
            </button>
            <button class="btn-action btn-edit" onclick="event.stopPropagation(); bukaModalEdit('${fasilitas.id}')" title="Edit">
              <i data-lucide="pencil"></i>
            </button>
            <button class="btn-action btn-delete" onclick="event.stopPropagation(); hapusFasilitas('${fasilitas.id}')" title="Hapus">
  ...
  `;
    ...
    for (let i = 1; i <= totalHalaman; i++) {
        ...
        if (i === 1 || i === totalHalaman || (i >= halamanSekarang - 1 && i <= halamanSekarang + 1)) {
            htmlPagination += `
        <button class="pagination-number ${i === halamanSekarang ? 'active' : ''}"
    ...
    // Tombol Next
    htmlPagination += `
    <button class="pagination-btn ${halamanSekarang === totalHalaman ? 'disabled' : ''}"
            onclick="gantiHalaman(${halamanSekarang + 1})"
...
function cariPasilitas(keyword) {
    if (!keyword || keyword.trim() === '') {
        ...
        daftarFasilitasFilter = daftarFasilitas.filter(fasilitas => {
            return fasilitas.nama.toLowerCase().includes(keywordLower) ||
                fasilitas.kategori.toLowerCase().includes(keywordLower) ||
                (fasilitas.alamat && fasilitas.alamat.toLowerCase().includes(keywordLower)) ||
                (fasilitas.deskripsi && fasilitas.deskripsi.toLowerCase().includes(keywordLower));
        });
    }
...
function resetFilter() {
    ...
    const checkboxes = document.querySelectorAll('#filterKategori input[type="checkbox"]');
...
function updateStatistik() {
    ...
    // Hitung per kategori
    const jumlahSekolah = daftarFasilitas.filter(f => f.kategori === 'Sekolah').length;
    const jumlahRumahSakit = daftarFasilitas.filter(f => f.kategori === 'Rumah Sakit').length;
...
function updateStatsDetail() {
    ...
    if (htmlStats === '') {
        htmlStats = '<p style="color: var(--text-muted); text-align: center;">Belum ada data</p>';
    }
